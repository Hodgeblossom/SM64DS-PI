...
struct BMD_File
{
	...
	// These three functions are called in SharedFilePtr::LoadBMD if numRefs == 1 after loading the file
	void InitPointers(); // before this is called, the pointers are offsets within the file
	CommonModelData* AddToArrayAndLoadTexAndPal(); // adds an entry for the file to commonModelDataArr, loads textures and palettes to VRAM and returns a pointer to the new entry
	void ShrinkAllocation(); // unloads textures and palettes from RAM by shrinking the allocation size to ramSize

	void LoadTexAndPal();

	void PrepareAnim(BMA_File& matChgFile);
	void PrepareAnim(BTA_File& texSrtFile);
	void PrepareAnim(BTP_File& texSeqFile);
};
