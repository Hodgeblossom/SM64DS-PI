...
struct MeshColliderBase // vtable at 0x02099388
{
	...
	virtual u32  DetectClsn(SphereClsn& sphere);
	...
	[[gnu::always_inline]]
	void EnableIfDisabled(Actor* actor)
	{
		if (!IsEnabled())
			Enable(actor);
	}
...
struct MeshCollider : MeshColliderBase
{
	...
	virtual u32  DetectClsn(SphereClsn& sphere) override;
...
extern MeshColliderBase* ACTIVE_MESH_COLLIDERS[24];
